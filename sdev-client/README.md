...
- The Core chat replacement is always enabled. It hides stock chat text, records the same incoming chat stream, and renders upper/lower messages with the game's own native chat font (its `ID3DXFont`) at the native CChat coordinates — so text matches native chat exactly and is drawn in the game's text codepage directly, while resolution changes, native vertical resize, and native scroll state remain aligned. Inline emoji/GIF tokens are still drawn as ImGui textured quads over that text.
